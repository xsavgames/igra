 ...
 *   Completes the quest, if Auto Reward is on, the script will give out the награду.
 *
 * Quest Fail QuestID
 *   Fails the quest.
 *
...
//---------------------------------------------------------------------------------------------
    ...
    Window_QuestInfo.prototype.drawQuestSteps = function(q) {
        ...
        this.questBitmap.textColor = this.systemColor(); 
        this.questBitmap.drawText("Шаги", 0, this.lineY, this.contentsWidth(), this.lineHeight());
        this.write();
        this.questBitmap.textColor = this.normalColor();
...
//---------------------------------------------------------------------------------------------
    ...
    Window_QuestFilter.prototype.initialize = function() {
        this.qFilters = [
            GameusScripts["Config"]["QuestSystem"]["All Word"] || "Все",
            GameusScripts["Config"]["QuestSystem"]["Progress Word"] || "В процессе",
            GameusScripts["Config"]["QuestSystem"]["Completed Word"] || "Завершеные",
            GameusScripts["Config"]["QuestSystem"]["Failed Word"] || "Проваленые",
        ];
        var xx = (GameusScripts["Config"]["QuestSystem"]["Reverse Layout"] || "false").toLowerCase() === "true" ? Graphics.boxWidth - 320 : 0;
